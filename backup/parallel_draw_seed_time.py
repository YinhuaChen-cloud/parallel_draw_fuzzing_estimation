...
REPEAT = 1 # 重复次数为 3
# ============================ seed-time 切换点
# TOTAL_TIME = 2606603000 # 单位执行次数 2606603000
# SPLIT_NUM = 72 * 60 # 分隔数量
...
def bug_time_worker(FUZZER, TARGET, thePROGRAM, TIME, task_count):
    ...
    for crash_file in files:
        ...
        if matches:
            ...
            if crash_time < SPLIT_NUM:
                ...
                cmd = [put]            
                assert(program_args[thePROGRAM] is not None)
                if program_args[thePROGRAM]:
                    cmd.append(program_args[thePROGRAM])
    ...
    for seed_file in files:
        ...
        if matches:
            ...
            # 如果是 +pat 种子，那么跳过，因为它大概率不触发新 bugs
            pat_matches = re.findall(r"\+pat", seed_file)
            assert(len(pat_matches) < 2)
            if pat_matches:
                continue
...
def main():
    ...
    for PROGRAM in PROGRAMS:
        ...
        plt.savefig('seed_time_' + PROGRAM + '_empty.svg', format='svg')  # 你可以指定文件格式，例如 'png', 'jpg', 'pdf', 'svg'
